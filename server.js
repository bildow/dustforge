...
app.get('/api/stripe/success', async (req, res) => {
  ...
  res.send('<html><head><meta charset="UTF-8"></head><body style="background:#08111a;color:#e7f1fb;font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0"><div style="text-align:center;max-width:500px;padding:2rem"><h1 style="color:#69c7b1">Payment Successful</h1><p>Your silicon identity has been activated with 90 Diamond Dust.</p>'+info+'<a href="https://demipass.com/vault-mobile.html" style="display:inline-block;margin-top:1.5rem;padding:0.8rem 2rem;background:#5b9bd5;color:#08111a;font-weight:700;border-radius:8px;text-decoration:none">Sign in to DemiPass &rarr;</a><p style="margin-top:0.75rem;font-size:12px;color:#6d8397">Sign in with the username and password you just chose.</p></div></body></html>');
});
...
function resolveSshTargetUser({ explicit, secret, context, override_reason }) {
  ...
  if (explicitStr) {
    ...
    if (bound && explicitStr !== secret.username) {
      ...
      return { ok: true, effective_user: explicitStr, override: { bound_username: secret.username, explicit: explicitStr, reason } };
    }
    return { ok: true, effective_user: explicitStr, override: null };
  }

  if (bound) {
    if (!shapeOk(secret.username)) return { ok: false, status: 400, error: 'invalid bound username on secret' };
    return { ok: true, effective_user: secret.username, override: null };
  }

  if (ctxDefault) {
    if (!shapeOk(ctxDefault)) return { ok: false, status: 400, error: 'invalid target_user_default on context' };
    return { ok: true, effective_user: ctxDefault, override: null };
  }
...
app.post('/api/blindkey/store', rateLimitStandard, billing.billingMiddleware(db, 'api_call_write', { cost: 0 }), (req, res) => {
  ...
  // Account binding (2026-08-24): optional at store time to keep migration incremental,
  // but strongly recommended for password secrets. Empty string = legacy/unbound.
  ...
  if (username !== undefined && username !== null) {
    ...
    if (u !== '' && !/^[a-zA-Z0-9._@+-]+$/.test(u)) {
      return res.status(400).json({ error: 'username: letters, digits, and . _ @ + - only (empty allowed to clear binding)' });
    }
    resolvedUsername = u;
  }
...
app.post('/api/blindkey/rotate-blind', rateLimitStandard, async (req, res) => {
  ...
  try {
    const scriptPath = `/tmp/.dp_rotate_${crypto.randomBytes(4).toString('hex')}.sh`;
    const scriptContent = '#!/bin/bash\necho ' + JSON.stringify(currentPassword) + ' | sudo -S bash -c "echo ' + sshUser + ':' + newPassword + ' | chpasswd" 2>/dev/null\necho ROTATED\n';
    fs.writeFileSync(scriptPath, scriptContent, { mode: 0o700 });

    const result = execSync(
      'SSHPASS=' + JSON.stringify(currentPassword) + ' sshpass -e ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 ' + sshUser + '@' + target_host + ' "bash -s" < ' + scriptPath,
    ...
    for (const ctx of oldContexts) {
      db.prepare('INSERT INTO blindkey_contexts (secret_id, context_name, action_type, target_url_pattern, target_host_pattern, allowed_by, max_uses, use_count) VALUES (?, ?, ?, ?, ?, ?, ?, 0)')
        .run(newSecret.id, ctx.context_name, ctx.action_type, ctx.target_url_pattern, ctx.target_host_pattern, ctx.allowed_by, ctx.max_uses);
    }
    ...
    try {
      db.prepare('INSERT INTO ticks (did, note, ip, tz, tick_type, tags) VALUES (?, ?, ?, ?, ?, ?)')
        .run(ownerDid, 'Blind rotation: ' + baseName + ' on ' + target_host + ' (' + (reason || 'exposed') + ')', req.ip || '', 'UTC', 'decision', JSON.stringify(['rotation:blind', 'host:' + target_host]));
    } catch(_) {}

    res.json({
      ok: true, old_ref: ref, old_status: 'revoked', new_ref: newSecret.ref_code, new_name: newName,
      target_host, target_user: sshUser, contexts_transferred: oldContexts.length,
      note: 'Password rotated. New password generated server-side and never entered any agent context.',
    });
  } catch (e) {
